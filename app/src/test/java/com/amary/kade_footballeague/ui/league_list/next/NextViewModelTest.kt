...
class NextViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = NextViewModel(apiRepository)
    }
    ...
    fun getNextMatch() {
        ...
        Mockito.`when`(apiRepository.getNextMatchEvent(id)).thenReturn(eventsResponse)

        val observer = mock<Observer<EventsResponse>>()
        viewModel.getNextMatch(id).observeForever(observer)
